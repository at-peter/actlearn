...
def tilecode_encoder_Decoder(p, v):
...
if __name__ == '__main__':
    ...
    for i in range(max_iteration):
        ...
        for j in range(max_step):
            p, v = observation_to_state(environment, observation)
            # select an action
            # FIXME: This should be replaced with a agent.policy 
            '''
            agent.policy(p,v)
                tiles = _mytiles(p,v)
                for tile in tiles: 
                    weight_vec.append(weights[tile])
                best action = np.argmax(weight_vec)
            '''
            if np.random.uniform(0, 1) < epsilon:
                # get random action
                action = np.random.choice(environment.action_space.n) # I dont get how this passes the proper reward 
            else:
                logits = q_table[p][v]
